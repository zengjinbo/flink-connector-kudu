...
import static com.psosuo.flink.kudu.table.utils.KuduTableUtils.toKuduFilterInfo;
...
public class KuduTableSource implements StreamTableSource<Row>,
    ...
    private static final Logger logger =LoggerFactory.getLogger(KuduTableSource.class);
    ...
    public TableSource<Row> applyPredicate(List<Expression> predicates) {
        ...
        while(predicatesIter.hasNext()) {
            ...
            if (kuduPred != null && kuduPred.isPresent()) {
                logger.debug("Predicate [{}] converted into KuduFilterInfo and pushed into " +
                    "KuduTable [{}].", predicate, tableInfo.getName());
                kuduPredicates.add(kuduPred.get());
                predicatesIter.remove();
            } else {
                logger.debug("Predicate [{}] could not be pushed into KuduFilterInfo for KuduTable [{}].",
