...
public abstract class AbstractSingleOperationMapper<T> implements KuduOperationMapper<T> {
     ...
     * @return Column value
     */
    public  PartialRow getField(T input, String column,int i, PartialRow partialRow) {
        partialRow.addObject(column, getField(input, i));
        return  partialRow;
    };
    ...
    public List<Operation> createOperations(T input, KuduTable table) {
        ...
        for (int i = 0; i < columnNames.length; i++) {
            getField(input, columnNames[i],i,partialRow);
        }
